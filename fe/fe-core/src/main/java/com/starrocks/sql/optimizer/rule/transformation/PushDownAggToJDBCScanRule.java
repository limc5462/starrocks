 ...
 *   <li>All aggregate functions must be in the supported whitelist (sum / count / max / min / avg).</li>
 ...
 *
 * *   After:
 *     LogicalJDBCScanOperator(table=ck_table, groupingKeys=[...], aggCalls={...})
 * </pre>
 */
public class PushDownAggToJDBCScanRule extends TransformationRule {

    /** Aggregate functions that can be safely pushed to ClickHouse. */
    ...
    private static final PushDownAggToJDBCScanRule INSTANCE = new PushDownAggToJDBCScanRule();

    public static PushDownAggToJDBCScanRule getInstance() {
        return INSTANCE;
    }

    private PushDownAggToJDBCScanRule() {
    ...
    public boolean check(OptExpression input, OptimizerContext context) {
        ...
        //TODO Do not fire if aggregation has already been pushed down.
        ...
        if (scan.getAggCalls() != null && !scan.getAggCalls().isEmpty()) {
            return false;
        }
    ...
    public List<OptExpression> transform(OptExpression input, OptimizerContext context) {
        ...
        newScan.setAggCalls(newAggregations);
