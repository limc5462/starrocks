...
public class ClickhouseSchemaResolver extends JDBCSchemaResolver {
     ...
     * mapping. However, the ClickHouse JDBC driver currently returns all aggregate
     * intermediate columns as String types by default. If we were to map these to their 
     * true underlying types (e.g., Int8, Decimal) here, it would cause cast exceptions 
     * in the execution engine because the actual ResultSet type (String) would conflict 
     * with the expected schema type. Addressing this requires fixing the inconsistent 
     * ResultSet reading mechanisms during query execution.
     */
